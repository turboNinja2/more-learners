...
type node_decision_tree = { split : int ; if_t : decision_tree ; if_f : decision_tree }
and  decision_tree = Node of node_decision_tree | Leaf of float_pair  (* num true, num false *)

let make_float_pair a b = { c_t = a ; c_f = b }
...
let predict_committee dts x =
  fold_right (fun (a,decision_tree) z -> z +. a *. predict decision_tree x) dts 0. /. fold_right (fun (a,_) z -> z+.a) dts 0.
...
let trim_tree_same decision_tree =
          ...
          (match t,f with
               Leaf p1, Leaf p2 ->
                 ...
                 let fp = p2.c_t /. (p2.c_t +. p2.c_f) in
                   ...
                   then Leaf (make_float_pair (p1.c_t +. p2.c_t) (p1.c_f +. p2.c_f))
                   else Node { split = n.split ; if_t = t; if_f = f }
             | _ -> Node { split = n.split ; if_t = t; if_f = f })
    | l -> l in
    trim_tree_same' decision_tree

(* for normal building, validEx None *)
let build_decision_tree (max_depth : int) (leaf_acc : float) (smooth : float) (validExO : int array option) (_F : int_array array) (_Y : bool array) (_W : float array) =
  ...
  let rec build_dt' depth c_t c_f =
    ...
    let p_f = 1. -. p_t in
      ...
      if c_t <= 0. || c_f <= 0. || min p_t p_f < leaf_acc || depth >= max_depth then Leaf (make_float_pair c_t c_f) else (
        (* build the tree *)
        match find_split_feature c_t c_f _F _Y _W used validEx with
            None -> Leaf (make_float_pair c_t c_f)
...
let build_bagged_decision_tree (size : int) (max_depth : int) (leaf_acc : float) (smooth : float) _ (_F : int_array array) (_Y : bool array) (_W : float array) =
  let _N = length _Y in
  ...
  let new_W   = make _N 0. in
    init size (fun _ ->
                 (* let maxW = fold_right max _W 0. in *)
                   ...
                   (1., build_decision_tree max_depth leaf_acc smooth (Some validEx) _F _Y new_W)
              )


let build_boosted_decision_tree (size : int) (max_depth : int) (leaf_acc : float) (smooth : float) (input_f : string) (_F : int_array array) (_Y : bool array) (_W : float array) =
  let _N = length _Y in
  let validEx = make _N 1 in
  let new_W   = copy _W in
  let sum_W   = fold_right (+.) _W 0. in
  let dts     = make size (0., Leaf (make_float_pair 0. 0.)) in
  (* let logit y = log (y /. (1. -. y)) in *)
  let h = open_in input_f in
  let mrl () = try Some (input_line h) with End_of_file -> None in
    dts.(0) <- (1., build_decision_tree max_depth leaf_acc smooth (Some validEx) _F _Y new_W);
      ...
      let error = ref 0. in
        ...
        if !error > 0. then (
          ...
          let sum  = ref 0. in
            ...
            dts.(i) <- (alpha, build_decision_tree max_depth leaf_acc smooth (Some validEx) _F _Y new_W);
        ) else (
          dts.(i) <- dts.(i-1);
        );
    done;
...
let build_single_decision_tree (max_depth : int) (leaf_acc : float) (smooth : float) _ (_F : int_array array) (_Y : bool array) (_W : float array) =
  [|1., build_decision_tree max_depth leaf_acc smooth None _F _Y _W |]
...
let print_tree out decision_tree =
...
let rec read_tree h = 
  let l = input_line h in
    match split_white l with
      ...
      | ["L" ; nt ; nf] -> Leaf (make_float_pair (float_of_string nt) (float_of_string nf))
      | _ -> failwith ("malformed line: '" ^ l ^ "'")

let load_model fp =
  let h = open_in fp in
  let size = int_of_string (input_line h) in
  let model = make size (0., Leaf (make_float_pair 0. 0.)) in
...
let () =
  ...
  let load_decision_tree    = ref None in
  let max_d      = ref 10 in
  let rho        = ref 0. in
  let minfc      = ref 1 in
  ...
  let _I = length Sys.argv in
    while !i < _I do
      ...
      else if !i < _I-1 && Sys.argv.(!i) = "-load"  then ( load_decision_tree    := Some (Sys.argv.(!i+1))               ; i := !i+2 )
  ...
  let input_f = match !input_f with None -> failwith usage | Some s -> s in
    (match !load_decision_tree with
         None -> (
           let learn = 
             match !boost_size, !bag_size with
                 None  , None   -> build_single_decision_tree 
               | Some n, None   -> build_boosted_decision_tree n
               | None  , Some n -> build_bagged_decision_tree n
